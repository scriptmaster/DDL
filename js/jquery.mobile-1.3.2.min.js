...
$(document).bind('pageinit', function(event){ 
	var url = $(event.target).data('url').split('/');
	...
	if(url == 'video.html') {
		...
		$.get('http://system-hostings.dev.wiredelta.com/ddl/home/video', function(html){
			var vi_content = $('#vi_content', html).html();
		...
		if(localStorage.video_url) {
			document.getElementById('playVideo').src = localStorage.video_url;
		}
	}
});

///video link screen size///
    ...
    $(window).resize(function() {
        var wi = $(window).width();
		 ...
		 if (wi <=320 && hi <=480){
			...
			$('img.resizeScreen').attr('src','img/320x480.png');
            }
			else
        if (wi <320 && hi <=568){
           
			$('img.resizeScreen').attr('src','img/640x1136.png');
            }
        else if (wi < 374){
            
			$('img.resizeScreen').attr('src','img/720x1280.png');
            }
			else if (wi <=720){
          
			$('img.resizeScreen').attr('src','img/720.png');
            }
        else if (wi <=1036){
          
			$('img.resizeScreen').attr('src','img/home_video.png');
            }
        else if (wi < 1200){
...
function myForm()
{
	...
	var message=$("#message").val();
      ...
      if(name == "")
      {
            $("#mail_status").html('<label>Enter your name.</label>');
            $("#name").focus();
      }
      else if(email == "")
      {
            $("#mail_status").html('<label>Enter ur mail ID.</label>');
            $("#email").focus();
      }
      else if(reg.test(email) == false)
      {
            $("#mail_status").html('<label>Enter a valid mail ID.</label>');
            $("#email").focus();
      }
	  
      else if(message == "")
      {
            $("#mail_status").html('<label>Enter ur comment to go.</label>');
            $("#message").focus();
      }
...
function memberForm()
{
  ...
  var details=$("#details").val();
      ...
      if(fname == "")
      {
            $("#mailstatus").html('<label>Enter your name.</label>');
            $("#fname").focus();
      }
      else if(mail == "")
      {
            $("#mailstatus").html('<label>Enter ur mail ID.</label>');
            $("#mail").focus();
      }
      else if(reg.test(mail) == false)
      {
            $("#mailstatus").html('<label>Enter a valid mail ID.</label>');
            $("#mail").focus();
      }
    
      else if(details == "")
      {
            $("#mailstatus").html('<label>Enter ur details.</label>');
            $("#details").focus();
      }

      else
      {
  var dataString='fname='+ fname + '&mail=' +  mail + '&details=' + details;
    var baseurl = $('.baseurl').val();
            $.ajax({
            url  : baseurl + 'home/member_send',
      data: dataString,
            type : "POST",
            dataType: 'json',
      success : function(data){
...
/////
	...
	// post answer
	function input(value,cmpq_id){

		var uuid = device && device.uuid? device.uuid: 1;
		var dataString = 'opt_id='+ value + '&q_id='+cmpq_id + '&uuid='+uuid;
		var baseurl = $('.baseurl').val();

		$.ajax({
			url  : baseurl + 'home/validate_answer',
			data: dataString,
			type : "POST",
			dataType: 'json',
			cache: false,
			...
			success : function(response){
				...
				navigator.notification.alert(response, null, 'Quiz', 'OK');
			}
		});
	}
	...
	function dis_able(id){
		if(id == 'id1'){
			$('#id2,#id3,#id4').prop('disabled', true);
                    ...
                    $('#id1,#id3,#id4').prop('disabled', true);
		}
		else if(id == 'id3') {
			$('#id2,#id1,#id4').prop('disabled', true);
		}
		else {
			$('#id2,#id3,#id1').prop('disabled', true);
		}
	}
